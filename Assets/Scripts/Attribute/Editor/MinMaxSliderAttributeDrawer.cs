...
public class MinMaxSliderDrawer : PropertyDrawer
{
    ...
    private Rect[] SplitRect(Rect rectToSplit, int n)
    {
        ...
        float dividedWidth = rectToSplit.width / n;

        for (int i = 0; i < n; i++)
            rects[i] = new Rect(rectToSplit.position.x + (i * dividedWidth), rectToSplit.position.y, dividedWidth, rectToSplit.height);

        int padding = 40;

        rects[0].width -= padding;
        rects[2].width -= padding;
