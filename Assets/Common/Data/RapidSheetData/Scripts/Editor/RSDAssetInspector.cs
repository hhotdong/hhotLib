...
namespace Lib.RapidSheetData
{
    ...
    public class RSDAssetInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Fetches all data Ids
            if(GUILayout.Button("Update data Ids"))
            {
                var rsdAsset = target as RSDAsset;
                if(rsdAsset != null)
                {
                    rsdAsset.UpdateDataIds();
                    EditorUtility.SetDirty(target);
                }
            }

            // Remote Initialize SO Managers
            if(GUILayout.Button("Remote Initialize SO Managers"))
            {
                var rsdAsset = target as RSDAsset;
                if (rsdAsset != null)
                {
                    rsdAsset.FetchRemoteInitializeData();
                    EditorUtility.SetDirty(target);
                }
            }
