...
namespace hhotLib.Save
{
    public abstract class SavableSO : ScriptableObject, ISavable
    {
        ...
        /// Important is that the data derived from this class must be dependencies of other Object.
        /// In other words, that data must be referenced by any Object(e.g. Any Object which is placed on your scenes where the data used).
        /// If not, OnEnable() method will not always be called so that this data cannot be updated.
        /// </summary>
        private void OnEnable()
        {
            //Debug.Log("SO OnEnable : " + this.name);
#if UNITY_EDITOR
        ...
        {
            //Debug.Log("SO OnDisable : " + this.name);
