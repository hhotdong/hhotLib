...
public class DayAndNightControl : MonoBehaviour {
	...
	public float SecondsInAFullDay = 10f; //in realtime, this is about two minutes by default. (every 1 minute/60 seconds is day in game)
	[Range(0,1)]
	public float currentTime = 0; //at default when you press play, it will be nightTime. (0 = night, 1 = day)
	[HideInInspector]
	public float timeMultiplier = 10.0f; //how fast the day goes by regardless of the secondsInAFullDay var. lower values will make the days go by longer, while higher values make it go faster. This may be useful if you're siumulating seasons where daylight and night times are altered.
	public bool showUI;
	...
	void UpdateLight()
	{
		...
		if (currentTime <= 0.2f) {
			Color currSkyColor = Color.Lerp(RenderSettings.ambientSkyColor, nightColors.skyColor, Time.deltaTime);
			Color currEquatorColor = Color.Lerp(RenderSettings.ambientEquatorColor, nightColors.equatorColor, Time.deltaTime);
			Color currHorizonColor = Color.Lerp(RenderSettings.ambientSkyColor, nightColors.horizonColor, Time.deltaTime);
		...
		if (currentTime > 0.2f && currentTime < 0.4f) {
			Color currSkyColor = Color.Lerp(RenderSettings.ambientSkyColor, dawnColors.skyColor, Time.deltaTime);
			Color currEquatorColor = Color.Lerp(RenderSettings.ambientEquatorColor, dawnColors.equatorColor, Time.deltaTime);
			Color currHorizonColor = Color.Lerp(RenderSettings.ambientSkyColor, dawnColors.horizonColor, Time.deltaTime);
			RenderSettings.ambientSkyColor = currSkyColor;
			RenderSettings.ambientEquatorColor = currEquatorColor;
			RenderSettings.ambientGroundColor = currHorizonColor;
		...
		if (currentTime > 0.4f && currentTime < 0.75f) {
			Color currSkyColor = Color.Lerp(RenderSettings.ambientSkyColor, dayColors.skyColor, Time.deltaTime);
			Color currEquatorColor = Color.Lerp(RenderSettings.ambientEquatorColor, dayColors.equatorColor, Time.deltaTime);
			Color currHorizonColor = Color.Lerp(RenderSettings.ambientSkyColor, dayColors.horizonColor, Time.deltaTime);
			RenderSettings.ambientSkyColor = currSkyColor;
			RenderSettings.ambientEquatorColor = currEquatorColor;
			RenderSettings.ambientGroundColor = currHorizonColor;
	...
	void OnGUI()
	{
        ...
        if(GUI.Button(new Rect(0.0F, 200.0F, 200.0F, 200.0F), "Toggle Day & Night" ))
        {
			isInclining = !isInclining;
		}

		if (GUI.Button(new Rect(0.0F, 400.0F, 200.0F, 200.0F), "Gen new Creature"))
		{
            for (int i = 0; i < bears.Length; i++)
            {
                if(bears[i].activeSelf == false)
                {
		...
		if (GUI.Button(new Rect(0.0F, 600.0F, 200.0F, 200.0F), "Camera rotate_x"))
		{
			if (Camera.main.transform.rotation.eulerAngles.x > 20.0F)
			{
				Camera.main.transform.rotation = Quaternion.Euler(10.0F, 0.0F, 0.0F);
			}
            else
            {
				Camera.main.transform.rotation = Quaternion.Euler(25.0F, 0.0F, 0.0F);
			}
		}
	}
}
